...
if __name__ == '__main__':
    ...
    model.eval()
    emb, y = model.encoder.get_embeddings(dataloader) #\ get embdeddings è creata su gin.py, a noi serve solo emb, non y!!
    ...
    for epoch in range(1, epochs+1):
        loss_all = 0
        model.train()
        for data in dataloader: #\ per ogni data in loader fa unsupervised learning con infograph
            data = data.to(device)
            optimizer.zero_grad()
    ...
    #\ faccio clustering k-means invece che accuracies!
        if epoch % log_interval == 0:  #\ se si entra nell'if fa predizione unsupervised (clustering) dell'embedding 
            ...
            import numpy as np
            # Converti la lista di embedding in un array numpy
            emb_array = np.array(emb)
            # Specifica il numero di cluster (k)
            k = 3
            # Crea un oggetto KMeans con il numero desiderato di cluster
            kmeans = KMeans(n_clusters=k, random_state=0)
            # Addestra il modello KMeans sugli embedding
            kmeans.fit(emb_array)
            # Ottieni le etichette di clustering assegnate a ciascun embedding
            labelsold = labels #\ questo è per vedere come cambiano le etichette ad ogni iterazione
            labels = kmeans.labels_

            # Ora 'labels' contiene l'etichetta di cluster assegnata a ciascun embedding
            print(labels)
            if labelsold is not None:
                print(labelsold-labels)
