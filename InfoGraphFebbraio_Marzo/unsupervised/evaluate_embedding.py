...
class LogReg(nn.Module):
    def __init__(self, ft_in, nb_classes):
        ...
        for m in self.modules():
            self.weights_init(m)  #\ inizializza i pesi

    def weights_init(self, m): #\ inizializza i pesi
...
def logistic_classify(x, y):
    ...
    for train_index, test_index in kf.split(x, y): #\ fa kfold con gli indici
    ...
    return np.mean(accs) #\ fa la media su tutti i k-fold!
...
def evaluate_embedding(embeddings, labels, search=True):
    labels = preprocessing.LabelEncoder().fit_transform(labels) #\ per trasformare le etichette in numeri!
    x, y = np.array(embeddings), np.array(labels)
    # print(x.shape, y.shape)

    logreg_accuracies = [logistic_classify(x, y) for _ in range(1)] #\trucchetto per poter usare list comprhension
    ...
    #/ commento gli ultimi modelli di classificazione qua sotto per runnare più velocemente il codice!
